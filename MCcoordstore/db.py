...
class User(UserMixin, db.Model):
    ...
    default_styleid = db.Column(db.Integer, db.ForeignKey("renderstyle.styleid", use_alter=True),
                                nullable=True)
    default_style = db.relationship("RenderStyle",
                                    foreign_keys=[default_styleid])
...
class PointOfInterest(db.Model):
    ...
    lowercasename = column_property(func.lower(name))
    ...
    coordtype = db.Column(Enum(CoordType,
                               default=str(CoordType.UNDEFINED.value),
                               nullable=False,
...
class Tag(db.Model):
    ...
    pois = db.relationship("PointOfInterest", secondary=poi_tag_association,
                           backref="tags")
